...
type IItemHandler = (id: number) => void
...
const NavItem: React.FC<{ item: any; level: number }> = ({ item, level }) => {
  ...
  if (item.external) {
    listItemProps = { w: 'a', href: item.url, target: itemTarget }
  }

  const itemHandler: IItemHandler = id => {
    ...
    >
      ...
      <ListItemText
        primary={
          <Typography
            variant={
              selectedItem.findIndex(id => id === item.id) > -1 ? 'h5' : 'body1'
            }
            color='inherit'
          >
            {item.title}
